...
const CreativePortfolio = () => {
  ...
  return (
    <div className="min-h-screen bg-navy-primary font-sans text-[#F8FAFC]">
      ...
      <AnimatePresence>
        {isContactOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setIsContactOpen(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="relative w-full max-w-md card-premium overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="p-6">
      ...
      <main className="relative z-10">
        {/* Hero – #0F172A */}
        <section className="min-h-[85vh] flex items-center pt-8 pb-20 px-4">
          <div className="container mx-auto max-w-6xl">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="text-center md:text-left">
                <p className="text-[#64748B] font-medium mb-2 text-sm uppercase tracking-wider">Hello, I'm</p>
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-[#F8FAFC] mb-4">
                  Jayash <span className="text-gradient-accent">Bhuyar</span>
                </h1>
                <p className="text-lg md:text-xl text-[#CBD5E1] mb-2 max-w-md">
                  Full-Stack Developer · AWS Cloud · Problem Solver
                </p>
                <p className="text-accent-blue font-medium mb-8 flex items-center gap-2 flex-wrap justify-center md:justify-start">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                  <motion.a href="https://drive.google.com/file/d/1INB7OoxPfbcJlQ9wh-eTu_wkydvMv5CC/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="btn-primary inline-flex items-center gap-2 px-5 py-2.5 font-medium">
                    <ExternalLink className="w-5 h-5" /> View Resume
                  </motion.a>
                  <motion.button type="button" onClick={() => setIsContactOpen(true)} className="btn-secondary inline-flex items-center gap-2 px-5 py-2.5 font-medium">
                    <Send className="w-5 h-5" /> Contact
                  </motion.button>
                </div>
                <TechStripMobile className="mt-6" />
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="flex justify-center md:justify-end">
                <div className="relative w-[260px] sm:w-[300px] md:w-[320px] md:translate-x-2">
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-10 md:mt-12">
              <FloatingTechLogos />
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-20 px-4 bg-navy-secondary">
          <div className="container mx-auto max-w-6xl">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="card-premium p-6 md:p-8 max-w-3xl mx-auto">
        ...
        {/* Skills – #0F172A */}
        <section className="py-20 px-4 bg-navy-primary">
          <div className="container mx-auto max-w-6xl">
            <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-2xl font-bold text-[#F8FAFC] text-center mb-4">Professional <span className="text-gradient-accent">Skills</span></motion.h2>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-center text-[#94A3B8] mb-10 text-sm">Technologies and tools I work with</motion.p>
            <div className="flex flex-wrap justify-center gap-6">
              {skills.map((skill, index) => (
                <motion.div key={skill.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08, duration: 0.5 }} className="card-premium p-6 w-64">
        ...
        <section className="py-20 px-4 bg-navy-secondary">
          <div className="container mx-auto max-w-6xl">
            <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-2xl font-bold text-[#F8FAFC] text-center mb-10"><span className="text-gradient-accent">Achievements</span></motion.h2>
            <div className="grid sm:grid-cols-2 gap-5 max-w-4xl mx-auto">
              {achievements.map((achievement) => (
                <motion.div key={achievement.title} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="card-premium p-5 flex gap-4">
                  {achievement.icon}
                  <div>
      ...
      <footer className="border-t border-white/5 bg-navy-section py-6">
        <div className="container mx-auto px-4 max-w-6xl text-center">
          <p className="text-[#64748B] text-sm">© {new Date().getFullYear()} Jayash Bhuyar. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
